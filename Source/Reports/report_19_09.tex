...
f(t) = -\frac{a}{bt + c} + d
\end{equation}
which $a = 4.741$, $b = 2.415$, $c = 67.565$, $d = 0.276$ and $t$ is the number of days since the first observation (t = 0). For these adjustments, the lack of fit tests were performed whose ANOVA tables are in tables \ref{tab:anova_sb231} and \ref{tab:anova_sb232}.
...
\begin{table}[hbt]
  ...
  \begin{tabular}{lrrrrr}
    \toprule
    & Degree of & Sum & Mean & Fisher & p-value\\
    & freedom & squared & squared & statistics &\\
    \cmidrule(lr){2-6}
    \textbf{Residual} & & & & &\\
    \textbf{Lack od fit} & & & & &\\
    \textbf{Pure error} & & & & &\\
